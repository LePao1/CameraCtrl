...
class Camera(object):
    def __init__(self, entry):
        ...
        self.w2c_mat[:3] = np.array(entry[7:]).reshape(3, 4)
    ...
    def get_batch(self, idx):
        ...
        if self.is_image:
            ...
            if isinstance(self.sample_stride, int):
                ...
                assert (self.sample_stride[0] >= 1) and (
                    self.sample_stride[1] >= self.sample_stride[0]
                )
                current_sample_stride = random.randint(
                    self.sample_stride[0], self.sample_stride[1]
                )
...
class RealEstate10KPose(Dataset):
    ...
    def __getitem__(self, idx):
        while True:
            try:
                video, video_caption, plucker_embedding, flip_flag, clip_name = (
                    self.get_batch(idx)
                )
                break
