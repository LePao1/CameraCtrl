...
def main(args):
    ...
    unet_additional_kwargs = (
        model_configs["unet_additional_kwargs"]
        if "unet_additional_kwargs" in model_configs
        else None
    )
    ...
    pipeline = get_pipeline(
        args.ori_model_path,
        args.unet_subfolder,
        args.image_lora_rank,
        args.image_lora_ckpt,
        unet_additional_kwargs,
        args.motion_module_ckpt,
        pose_encoder_kwargs,
        attention_processor_kwargs,
        noise_scheduler_kwargs,
        args.pose_adaptor_ckpt,
        args.personalized_base_model,
        f"cuda:{gpu_id}",
    )
    ...
    plucker_embedding = (
        ray_condition(K, c2ws, args.image_height, args.image_width, device="cpu")[0]
        .permute(0, 3, 1, 2)
        .contiguous()
    )  # V, 6, H, W
    ...
    if args.visualization_captions.endswith(".json"):
        ...
        captions = (
            json_file["captions"] if "captions" in json_file else json_file["prompts"]
        )
    ...
    prompts_per_gpu = [
        N + 1 if gpu_id < remainder else N for gpu_id in range(args.n_procs)
    ]
    low_idx = sum(prompts_per_gpu[:gpu_id])
    high_idx = low_idx + prompts_per_gpu[gpu_id]
    prompts = captions[low_idx:high_idx]
    negative_prompts = (
        negative_prompts[low_idx:high_idx] if negative_prompts is not None else None
    )
    specific_seeds = (
        specific_seeds[low_idx:high_idx] if specific_seeds is not None else None
    )
    print(
        f"rank {rank} / {torch.cuda.device_count()}, number of prompts: {len(prompts)}"
    )
    ...
    for local_idx, caption in tqdm(enumerate(prompts)):
        ...
        sample = pipeline(
            prompt=caption,
            negative_prompt=negative_prompts[local_idx]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ori_model_path", type=str, help="path to the sd model folder"
    )
    parser.add_argument(
        "--unet_subfolder", type=str, help="subfolder name of unet ckpt"
    )
    parser.add_argument(
        "--motion_module_ckpt",
        type=str,
        help="path to the animatediff motion module ckpt",
    )
    ...
    parser.add_argument(
        "--pose_adaptor_ckpt",
        default=None,
        help="path to the camera control model ckpt",
    )
    parser.add_argument("--model_config", type=str)
    parser.add_argument("--num_inference_steps", type=int, default=25)
    parser.add_argument("--guidance_scale", type=float, default=14.0)
    parser.add_argument(
        "--visualization_captions", required=True, help="prompts path, json or txt"
    )
    parser.add_argument(
        "--use_negative_prompt",
        action="store_true",
        help="whether to use negative prompts",
    )
    parser.add_argument(
        "--use_specific_seeds",
        action="store_true",
        help="whether to use specific seeds for each prompt",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1280,
        help="the width of the video used to extract camera trajectory",
    )
    parser.add_argument(
        "--original_pose_height",
        type=int,
        default=720,
        help="the height of the video used to extract camera trajectory",
    )
    parser.add_argument("--n_procs", type=int, default=8)

    # DDP args
    parser.add_argument(
        "--world_size", default=1, type=int, help="number of the distributed processes."
    )
    parser.add_argument(
        "--local_rank",
        type=int,
